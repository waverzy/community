...
wxUtils.downloadImages = function download (serverId, callback) {
    ...
    var filename = 'images/' + Date.now() + '.jpeg';
    request.get(api_url).pipe(fs.createWriteStream(filename)).on('close', function () {
        callback(filename);
    });
};
