...
wxUtils.unifiedOrder = function (body, fee, ip, callback) {
    ...
    var stringA = 'appid=' + params.appid + '&body=' + params.body + '&limit_pay=' + params.limit_pay + '&mch_id=' + params.mch_id
                + '&nonce_str=' + params.nonce_str + '&notify_url=' + params.notify_url + '&out_trade_no=' + params.out_trade_no
...
wxUtils.orderNew = function (close, orderId, body, fee, ip, callback) {
    if(close) {
        var oid = orderId || '';
        if(oid === '') {
        ...
        this.unifiedOrder(body, fee, ip, callback);
    }
};
